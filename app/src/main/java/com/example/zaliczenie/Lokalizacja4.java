...
public class Lokalizacja4 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lokalizacja4);

        //screen
        goBackButton = findViewById(R.id.goBackButton4);
        latTextView = findViewById(R.id.latTextView3);
        lonTextView = findViewById(R.id.lonTextView3);
        infoTextView = findViewById(R.id.infoTextView4);
        ...
        databaseReference.child("lat4").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful() && !String.valueOf(task.getResult().getValue()).equals("null"))
                {
                    ...
                    databaseReference.child("long4").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            if (task.isSuccessful() && !String.valueOf(task.getResult().getValue()).equals("null"))
                            {
                                ...
                                Geocoder geocoder = new Geocoder(Lokalizacja4.this, Locale.getDefault());
                                try {
                                    ...
                                    //{
                                    //    Toast.makeText(Lokalizacja4.this, "działa", Toast.LENGTH_SHORT).show();
                                    //}
                                } catch (IOException e) {
                                    e.printStackTrace();
                                }
                            ...
                            {
                                Toast.makeText(Lokalizacja4.this, "Najpierw musisz zapisać swoją lokalizację!",
        ...
        databaseReference.child("lat4").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(Lokalizacja4.this, "Wystąpił błąd", Toast.LENGTH_SHORT).show();
                }
                else {
                    if (String.valueOf(task.getResult().getValue()).equals("null"))
                    {
                        Toast.makeText(Lokalizacja4.this,"Musisz najpierw zapisać swoją lokalizację!", Toast.LENGTH_LONG).show();
                    }
                    else
                    {
