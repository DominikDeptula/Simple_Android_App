...
public class Lokalizacja3 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lokalizacja3);

        //screen
        goBackButton = findViewById(R.id.goBackButton3);
        latTextView = findViewById(R.id.latTextView4);
        lonTextView = findViewById(R.id.lonTextView2);
        infoTextView = findViewById(R.id.infoTextView3);
        ...
        databaseReference.child("lat3").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful() && !String.valueOf(task.getResult().getValue()).equals("null"))
                {
                    ...
                    databaseReference.child("long3").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            if (task.isSuccessful() && !String.valueOf(task.getResult().getValue()).equals("null"))
                            {
                                ...
                                Geocoder geocoder = new Geocoder(Lokalizacja3.this, Locale.getDefault());
                                try {
                                    ...
                                    //{
                                    //    Toast.makeText(Lokalizacja3.this, "działa", Toast.LENGTH_SHORT).show();
                                    //}
                                } catch (IOException e) {
                                    e.printStackTrace();
                                }
                            ...
                            {
                                Toast.makeText(Lokalizacja3.this, "Najpierw musisz zapisać swoją lokalizację!",
        ...
        databaseReference.child("lat3").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(Lokalizacja3.this, "Wystąpił błąd", Toast.LENGTH_SHORT).show();
                }
                else {
                    if (String.valueOf(task.getResult().getValue()).equals("null"))
                    {
                        Toast.makeText(Lokalizacja3.this,"Musisz najpierw zapisać swoją lokalizację!", Toast.LENGTH_LONG).show();
                    }
                    else
                    {
