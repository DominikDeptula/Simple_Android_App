...
public class Lokalizacja1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lokalizacja1);

        //screen
        goBackButton = findViewById(R.id.goBackButton1);
        latTextView = findViewById(R.id.latTextView);
        lonTextView = findViewById(R.id.lonTextView1);
                    ...
                    databaseReference.child("long1").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            if (task.isSuccessful() && !String.valueOf(task.getResult().getValue()).equals("null"))
                            {
                                ...
                                Geocoder geocoder = new Geocoder(Lokalizacja1.this, Locale.getDefault());
                                try {
                                    ...
                                    //{
                                    //    Toast.makeText(Lokalizacja1.this, "działa", Toast.LENGTH_SHORT).show();
                                    //}
                                } catch (IOException e) {
                                    e.printStackTrace();
                                }
                            ...
                            {
                                Toast.makeText(Lokalizacja1.this, "Najpierw musisz zapisać swoją lokalizację!",
        ...
        databaseReference.child("lat1").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(Lokalizacja1.this, "Wystąpił błąd", Toast.LENGTH_SHORT).show();
                }
                else {
                    if (String.valueOf(task.getResult().getValue()).equals("null"))
                    {
                        Toast.makeText(Lokalizacja1.this,"Musisz najpierw zapisać swoją lokalizację!", Toast.LENGTH_LONG).show();
                    }
                    else
                    {
