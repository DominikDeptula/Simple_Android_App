...
public class Lokalizacja2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lokalizacja2);

        //screen
        goBackButton = findViewById(R.id.goBackButton2);
        latTextView = findViewById(R.id.latTextView2);
        lonTextView = findViewById(R.id.lonTextView);
        infoTextView = findViewById(R.id.infoTextView2);
        ...
        databaseReference.child("lat2").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful() && !String.valueOf(task.getResult().getValue()).equals("null"))
                {
                    ...
                    databaseReference.child("long2").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            if (task.isSuccessful() && !String.valueOf(task.getResult().getValue()).equals("null"))
                            {
                                ...
                                Geocoder geocoder = new Geocoder(Lokalizacja2.this, Locale.getDefault());
                                try {
                                    ...
                                    //{
                                    //    Toast.makeText(Lokalizacja2.this, "działa", Toast.LENGTH_SHORT).show();
                                    //}
                                } catch (IOException e) {
                                    e.printStackTrace();
                                }
                            ...
                            {
                                Toast.makeText(Lokalizacja2.this, "Najpierw musisz zapisać swoją lokalizację!",
        ...
        databaseReference.child("lat1").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(Lokalizacja2.this, "Wystąpił błąd", Toast.LENGTH_SHORT).show();
                }
                else {
                    if (String.valueOf(task.getResult().getValue()).equals("null"))
                    {
                        Toast.makeText(Lokalizacja2.this,"Musisz najpierw zapisać swoją lokalizację!", Toast.LENGTH_LONG).show();
                    }
                    else
                    {
        ...
        databaseReference.child("long1").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(Lokalizacja2.this, "Wystąpił błąd", Toast.LENGTH_SHORT).show();
